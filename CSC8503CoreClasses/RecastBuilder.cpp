...
void RecastBuilder::BuildNavMesh(std::vector<GameObject*> objects, PhysicsSystem* physics) {
	mSizeSet = false;
	...
	if (physics) {
		physics->SetNewBroadphaseSize(Vector3(bmax[x] - bmin[x], bmax[y] - bmin[y], bmax[z] - bmin[z]));
	}
	mSizeSet = true;
