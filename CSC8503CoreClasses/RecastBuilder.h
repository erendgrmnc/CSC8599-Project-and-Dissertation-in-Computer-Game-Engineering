...
namespace NCL {
	namespace CSC8503 {
		...
		public:
			RecastBuilder();
			~RecastBuilder();
			// Pass the PhysicsSystem whose broadphase should be resized to the
			// generated nav-mesh bounds (previously fetched via the now-removed
			// LevelManager singleton). May be null to skip the resize.
			void BuildNavMesh(std::vector<GameObject*> objects, PhysicsSystem* physics = nullptr);
