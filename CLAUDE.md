...
> - **The arrival clamp is correct and wired in** (fixed 2026-08-26; this bullet previously said the body was wrong, which it was). `CalculateIncomingObjectOffsetPosition` is a thin adapter over `NCL::Interaction::ClampIntoRegion` (`DistributedSystemCommonFiles/RegionOwnership.h`), which lives beside `OwningServerFor` and is asserted against it at both 2- and 4-server partitions in `tools/InteractionTests/RegionOwnershipTests.cpp`. `ApplyIncomingObject` now **applies** the result — to `position` and `predictedPosition` both — rather than computing and discarding it. The old body kept its own copy of the bounds and clamped Z with an *inclusive* upper bound, which stopped matching `IsObjectInBorder` once that delegated to the half-open `OwningServerFor`: on an interior Z seam it returned a coordinate a different server owns. Masked at 2 servers, live at 4, and measured — `hoClamp` on server 1 went 0 → 2 reproducibly, and on no other server. **`hoClamp` changed meaning**: it counted arrivals that *would* have been moved and now counts arrivals that *were*, so figures either side of 2026-08-26 are not comparable.
> - **Handoff transfers are lossless, and the ownership gap closes below the pacing budget.** The ack path is live: the receiver acks on acceptance, the sender holds the transfer in custody until that ack arrives, resending on a wall-clock deadline and reclaiming only once the peer link is gone - never on a bare timeout, which cannot tell "never arrived" from "arrived and the receiver is slow" (`DistributedSystemCommonFiles/HandoffCustody.h`, `ServerWorldManager::FlushPendingTransfers`). **A resend is now recognised by transfer identity** - `(senderServerID, mSenderTick)`, both already on the wire - so one arriving after the object was handed *onward* is rejected rather than re-installed as a second copy (backlog item 15; 3,900 resends produced 3,906 rejected duplicate arrivals and no survivors).
>
>   **`--handoff-lookahead` defaults to 8 since 2026-08-26**, derived from a 60-run sweep. The sender releases at `senderTick + L` and the receiver installs on the same tick, so ownership changes atomically and the gap falls from up to 98% of ticks to 0-0.11%. Three things that sweep measured, none obvious:
>   - **It is a threshold, not a dial.** L = 2 is *worse* than L = 0 (~1776 gap ticks of 1800 on every repeat, against L = 0's bimodal 87/~1750), because 16.7 ms is below delivery latency and ~60% of arrivals miss their slot. Do not set a small non-zero value "to be safe".
>   - **Above the threshold every residual gap is a late arrival**, one for one across 18 runs.
>   - **It does not hold above the pacing budget.** At 8,000 objects (`phys_p95` 10.2-10.4 ms against a budget of 8.33 ms) the gap is ~1776 of 1800 and **no lookahead closes it** - 8, 16, 32 and 64 all sit at ~1600-1780. Tick-numbered scheduling is a shared clock only while both servers hold pace; this is the same tick-epoch divergence that bounds halo scheduling. The guarantee is conditional on **load**, not on a non-default flag.
>
>   The harness must not force the flag: `measure.ps1` uses `-1` to mean "do not pass it, let the server default stand". It previously passed `--handoff-lookahead 0` unconditionally, which silently ran every measurement at 0 while the code shipped 8.
