...
The Python analysis tooling (`tools/analyse.py`, `tools/gate-compare.py`) has its own unit tests,
standard-library only, runnable on the same machine that produces the data with no environment setup:

```powershell
python tools/test_analyse.py
python tools/test_gate_compare.py
```

Both are green as of this branch. Neither was previously mentioned here — only in the phase plan
that added them.
...
> - `CalculateIncomingObjectOffsetPosition` (`ServerWorldManager.cpp:440`) — note the name has no "ed"; `docs/SPATIAL-PARTITIONING.md` describes it as live. It **is** called now — from `ApplyIncomingObject`, on every non-reclaim arrival — but **only to observe**: the clamp is computed and the result is *discarded*, incrementing `hoClamp` when it would have moved the object. So incoming handoffs still get no positional nudge; what changed is that the codebase measures how often one would be needed, and that measurement shows the case is reachable under load (`hoClamp` fires on the 8,000-object and rebalancing runs) — **and also on the correctness-budget healthy-path configuration when `--halo-width` is on**: server 0 reported `hoClamp` 7–8 across four pre-change repeats and 8, 8, 8 across three gate repeats (`docs/superpowers/results/2026-08-23-A-instrumentation.md:152`), likely because an arriving object is promoted from an existing halo shadow rather than constructed, giving its arrival position a different code path; every run behind the original "never on healthy-path" claim had the halo off. **The body is NOT correct** (corrected 2026-08-23; this line previously claimed it was). It clamps Z with an *inclusive* upper bound, and its own comment says the bounds "mirror `IsObjectInBorder` exactly — half-open on X, closed on Z". That mirroring was true before the ownership unification and is false now: `IsObjectInBorder` delegates to `OwningServerFor`, which is half-open on **both** axes, so on an interior Z seam the clamp yields a position this server does not own — the disowned-object case the unification exists to prevent. It is masked at 2 servers (the split is 1-D, `maxZ == worldMaxZ`, and the closed-outer-edge exception applies) and reachable at 4 (`CalculateServerBorders` builds a 2×2 grid), and inert today only because the result is discarded. Earlier problems, since fixed: it returned a stack local by reference, commented out both X branches, and floored Z identically in both directions. Fix the Z bound *before* wiring it up; wiring it up moves incoming objects and so changes measured handoff behaviour — that belongs with the ownership unification, not as a drive-by. See `docs/superpowers/specs/2026-08-23-backlog-completion-design.md` §5.3.
> - **Handoff transfers are lossless; the ownership gap is not closed.** These are two different claims, and this file used to run them together. The ack path is live: the receiver acks on acceptance, and the sender holds the transfer packet in custody until that ack arrives, resending it on a wall-clock deadline and reclaiming the object only once the peer link itself is gone - never on a bare timeout, because a timeout cannot tell "the receiver never got it" from "the receiver got it and is slow" (`CSC8503CoreClasses/DistributedSystemCommonFiles/HandoffCustody.h`, `ServerWorldManager::FlushPendingTransfers`). An outstanding transfer is now visible (`hoCustody`) instead of silently lost, and `analyse.py` sums `owned_objects` per tick and fails on a gap or a double-owner. **The gap itself remains, and it is the default behaviour, not an edge case**: ownership only changes atomically - the sender releasing at the same `senderTick + lookahead` tick the receiver installs on - when `--handoff-lookahead > 0`. The default is 0, where `ScheduleOutgoingObject` releases on send and nobody owns the object for one network round trip. Before the lookahead mechanism existed, 1,397 of 1,800 ticks on a uniform run had an object owned by nobody, up to 33 at once, while `conservation_delta` and `ho_parity_delta` both read 0 - end-of-run totals cannot see it.
> - **Snapshots are sent at 60 Hz** (1 full : 5 delta → 10 Hz full, 50 Hz delta). The inline comment that used to claim 20 Hz has been corrected; `docs/NETWORKING.md:86` and `docs/ARCHITECTURE.md:43` state `1.0f/60.f`.
> - **The halo band is published once per tick.** It used to be published once per *loop iteration*, which is not the same thing: the drain phase runs the loop without stepping the world, so any run ending with objects still inside the band republished it continuously with the tick counter frozen — `haloSent` of ~180,000 against ~1,900 for the same run gated, differing between the two servers because each spun at its own rate. That broke invariant I8 (the two servers resolved different contact counts) as well as inflating bandwidth. Fixed; `docs/EVALUATION.md` §4.1 records which measurements it reached and the re-validation of those that it did.
