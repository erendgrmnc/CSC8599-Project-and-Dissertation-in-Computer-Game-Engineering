...
| Game Server | `--headless`, `--fixed-step`, `--seed N`, `--workload seam\|shuttle\|uniform\|headon`, `--metrics-dir`, `--metrics-capacity`, `--run-seconds`, `--run-ticks`, `--handoff-delay-ticks N`, `--handoff-lookahead N`, `--halo-width W`, `--halo-lookahead N`, `--halo-reliable` — **not passed directly**, see below |
